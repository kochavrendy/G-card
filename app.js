...
function updateCoinUI(){
  try{
    if(btnCoin){
      ...
      // 連続で同じ面でもアニメが再起動するようクラスをいったん除去してリフロー
      if(lastCoin){
        btnCoin.classList.remove('coin-flash');
        void btnCoin.offsetWidth;
        btnCoin.classList.add('coin-flash');
      }
    }
  }catch(e){}
...
function rebuildLibGradeOptions(){
  try{
    const container = document.getElementById('libGradeGroup');
    if(!container) return;
    ...
    // 既存のトグルボタンを削除してラベルだけ残す
    container.querySelectorAll('.filterBtn').forEach(b=>b.remove());
    arr.forEach(g=>{
      const btn=document.createElement('button');
      btn.type='button';btn.className='filterBtn';
      btn.dataset.filter='grade';btn.dataset.value=g;
      btn.textContent=g;
      if(libGradeSelected.has(g)) btn.classList.add('active');
...
function snapCardToZone(card, z){
  ...
  // ゾーン中央にぴったり重ねる（オフセットなし）
  card.x = z.x*bw + (z.w*bw - w)/2;
  card.y = z.y*bh + (z.h*bh - h)/2;
}
...
// フィルタートグルボタン：クリックで選択状態をトグル（複数選択・OR）
document.getElementById('libMetaBar')?.addEventListener('click',e=>{
